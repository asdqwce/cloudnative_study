...
install_linux_engine_cert() {
  ...
  if [ -f "${target}" ] && cmp -s "${cache_ca}" "${target}"; then
    log "Docker engine cert already installed: ${target}"
    return 0
  fi
...
install_windows_docker_desktop_cert() {
  ...
  if ! powershell.exe -NoProfile -ExecutionPolicy Bypass -Command \
    ...
    >/dev/null 2>&1; then
    log "warning: failed to import CA into Windows CurrentUser Root certificate store."
    log "continuing because Docker user/engine certs were already installed."
    log "if docker push reports x509 errors, import ${cache_ca} into Windows CurrentUser Root manually and restart Docker Desktop."
    return 0
  fi
