...
def handle_business_event(db: Session, payload: dict) -> Notification:
    ...
    processed_event = db.query(ProcessedEvent).filter(ProcessedEvent.event_id == event.eventId).one_or_none()
    if processed_event is not None:
        notification = db.get(Notification, processed_event.notification_id)
    ...
    db.add(notification)
    db.flush()
    db.add(ProcessedEvent(event_id=event.eventId, notification_id=notification.id))
    db.commit()
